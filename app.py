...
if not df_processed.empty:
    ...
    min_hora = df_processed['Hora_dt'].min().time() if pd.notna(df_processed['Hora_dt'].min()) else datetime.time(0, 0)
    max_hora = df_processed['Hora_dt'].max().time() if pd.notna(df_processed['Hora_dt'].max()) else datetime.time(23, 59)
    ...
    if patente_filter:
        df_filtrado = df_filtrado[df_filtrado['Patente'].astype(str).str.contains(patente_filter.strip(), case=False, na=False)]

    if start_time and end_time:
        df_filtrado = df_filtrado[df_filtrado['Hora_dt'].notna()]
        df_filtrado = df_filtrado[(df_filtrado['Hora_dt'].dt.time >= start_time) & (df_filtrado['Hora_dt'].dt.time <= end_time)]
    ...
    if df_filtrado.empty:
        ...
        st.divider()
        st.markdown(f"### 📊 Métricas: {', '.join(map(str, vias_seleccionadas))} | Sentido: {', '.join(map(str, sentido_seleccionado))}")
        ...
        output_cols = ['Hora', 'Vía', 'Patente', 'TAG', 'Sentido', 'Tránsito', 'Estado', 'Descripción Original']
        ...
        st.download_button("⬇️ Exportar CSV", data=csv_data, file_name="telepase_filtrado.csv", mime="text/csv")

        try:
            buffer = io.BytesIO()
            with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
                df_filtrado[output_cols].to_excel(writer, index=False, sheet_name='Telepase')
            st.download_button("⬇️ Exportar Excel", data=buffer.getvalue(), file_name="telepase_filtrado.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
        except Exception as e:
